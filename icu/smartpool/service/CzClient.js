...
class CzClient {
    ...
    async listSymbol() {
        ...
        contractList = contractList.data.symbols
            // 非USDT币对、非数字货币币对
            .filter(ele => 'USDT' === ele.quoteAsset && "TRADING" === ele.status && !this.exSet.includes(ele.symbol))
            // 新上市合约币对剔除
            .slice(0, -20)
            .map(ele => ele.symbol)
        // 过滤掉交易量小于10M的合约币对
        vols = await Promise.all(contractList.map(async symbol => {
        ...
        contractList = contractList.filter((_, i) => vols[i] > 10000000);
        ...
        spotList = spotList.data.symbols
            ...
            .map(ele => ele.symbol)
        // 过滤掉交易量小于5M的现货币对
        vols = await Promise.all(spotList.map(async symbol => {
            symbol = symbol.endsWith('BTC') ? symbol.replace('BTC', 'USDT') : symbol.endsWith('ETH') ? symbol.replace('ETH', 'USDT') : symbol;
            let rlt = await axios.get('https://api.binance.com/api/v3/ticker/24hr?symbol=' + symbol, {});
            return rlt.data.quoteVolume;
        }))
        // 交易量大于300w 平均交易量10M、因交易量过低过滤掉约55%
        spotList = spotList.filter((_, i) => vols[i] > 3000000);
